    ...
    struct NodePrinter {
        ...
        void PrintSyntaxList(SyntaxListUntyped* list) {
            ...
            if (list->size > 0) {
                ...
                int32 startTokenId = list->array[0]->GetStartTokenId();
                int32 endTokenId = list->array[list->size - 1]->GetEndTokenId();
        ...
        void PrintSeparatedSyntaxList(SeparatedSyntaxListUntyped* list) {
            ...
            if (list->itemCount > 0) {
                ...
                if (list->separatorCount == 0) {
                    ...
                    int32 itemToken = list->items[list->itemCount - 1]->GetEndTokenId();
                    if (s.GetId() > itemToken) {
                        PrintLineRange(list->items[0]->GetStartTokenId(), s.GetId());
                    }
                    else {
                        PrintLineRange(list->items[0]->GetStartTokenId(), itemToken);
                    }
                }
        ...
        void PrintLineRange(SyntaxBase* syntaxBase) {
            PrintLineRange(syntaxBase->GetStartTokenId(), syntaxBase->GetEndTokenId());
        }
